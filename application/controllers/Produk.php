...
class Produk extends CI_Controller
{
    ...
    public function checkout()
    {
        if (count($this->cart->contents()) == 0) {
            ...
            if ($this->session->email) {
                $data['title'] = 'Kaisar Products - Pembayaran';
    ...
    {   
        ...
        $data['title'] = 'Kaisar Products - Pencarian '.ucwords($nama);
    ...
    public function cari_produk($keyword = null)
    {
		...
		$data['tags'] = $this->Produk_Model->getAllTags();
		$data['cat'] = $this->Produk_Model->getAllCat();
        $data['title'] = 'Kaisar Products - Pencarian '. str_replace('%20',' ',ucwords($keyword));
