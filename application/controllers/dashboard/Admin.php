...
class Admin extends CI_Controller
{
  ...
  public function add_product()
  {
    ...
    $this->form_validation->set_rules('ket_produk', 'Keterangan Produk', 'min_length[20]');
  ...
  {
    $this->form_validation->set_rules('Varian', 'Nama Varian', 'min_length[3]|max_length[100]|required|trim|alpha_numeric_spaces');
   
    if ($this->form_validation->run() == false) {
      $data['title'] = 'Tambah Varian Produk - Admin';
      $data['VarianAktif'] = $this->Admin_Model->getAllVarian(1);
      $data['VarianNonaktif'] = $this->Admin_Model->getAllVarian(0);
      $this->load->view('admin/product/addVarian', $data);
    } else {
      $data = [
        'Varian' => clean(ucwords($this->input->post('Varian', true))),
        'active' => 1
      ];
      if ($this->Admin_Model->inputVarianProduk($data) > 0) {
        $this->freeM->getSweetAlert('message', 'Horay!', 'Varian Produk berhasil ditambahkan.', 'success');
      } else {
        $this->freeM->getSweetAlert('message', 'Upss!', 'Varian Produk gagal ditambahkan.', 'error');
      }
      redirect('dashboard/admin/add_Varian');
    }
  }

  public function deactive_Varian($id = null)
  {
    if ($id != null) {
      if ($this->Admin_Model->updateStatusVarian(decrypt_url($id), 0)) {
        $this->freeM->getSweetAlert('message', 'Horay!', 'Varian Produk berhasil dinonaktifkan.', 'success');
      } else {
        $this->freeM->getSweetAlert('message', 'Upss!', 'Varian Produk gagal dinonaktifkan.', 'error');
      }
      redirect('dashboard/admin/add_Varian');
    } else {
      redirect('dashboard/admin/add_Varian');
  ...
  public function reactive_Varian($id = null)
  {
    if ($id != null) {
      if ($this->Admin_Model->updateStatusVarian(decrypt_url($id), 1)) {
  ...
  {
    if ($id != null) {
      if ($this->Admin_Model->deleteVarian(decrypt_url($id))) {
  ...
  {
     $this->form_validation->set_rules('nama_Varian_new', 'Nama Varian', 'required|trim|min_length[3]|max_length[100]|alpha_numeric_spaces');

    if ($this->form_validation->run() == false) {
      $this->freeM->getSweetAlert('message', 'Upss!', 'Tag Produk gagal diupdate. Perbaiki data inputan!', 'error');
      redirect('dashboard/admin/add_Varian');
    } else {
      $idVarian =  $this->input->post('id_Varian_new', true);
      $data = [
        'Varian' => ucwords($this->input->post('nama_Varian_new', true)),
        'active' => 1
      ];
      if($this->Admin_Model->updateVarian($idVarian, $data)){
  ...
  public function add_category()
  {
    ...
    $this->form_validation->set_rules('ket_cat', 'Keterangan Kategori', 'required|trim|min_length[8]|max_length[255]');
    if ($this->form_validation->run() == false) {
  ...
  public function updateCat()
  {
    ...
    $this->form_validation->set_rules('ket_cat_new', 'Keterangan Kategori', 'required|trim|min_length[8]|max_length[255]');
    if ($this->form_validation->run() == false) {
