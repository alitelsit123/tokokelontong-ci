...
class Admin_Model extends CI_Model
{
    ...
    // @begin Varian Produk
    public function getAllVarian($status)
    {
        $this->db->where('active', $status);
        return $this->db->get('Varian')->result_array();
    }

    public function inputVarianProduk($data)
    {
        $this->db->insert('Varian', $data);
        return $this->db->affected_rows();
    }

    public function updateVarian($id, $data)
    {
        $this->db->where('id_Varian', $id);
        $this->db->update('Varian', $data);
        return $this->db->affected_rows();
    }

    public function updateStatusVarian($id, $status)
    {
        $this->db->where('id_Varian', $id);
        $this->db->update('Varian', ['active' => $status]);
        return $this->db->affected_rows();
    }

    public function deleteVarian($id)
    {
        $this->db->delete('Varian', ['id_Varian' => $id]);
        return $this->db->affected_rows();
    }
    // @end Varian Produk
